    ...
    return todos.filter((todo) => {
      ...
      if (excludeId) {
        ...
        const wouldCreateCycle = (
          ...
          return todo.dependencies.some((d) =>
            wouldCreateCycle(d.id, targetId)
          );
        };
    ...
    <div className="min-h-screen bg-gradient-to-b from-orange-500 to-red-500 flex flex-col items-center p-4">
      <div className="w-full max-w-4xl">
        ...
        {showGraph && (
          ...
          >
            <div
              className="bg-white rounded-lg p-6 max-w-[95vw] max-h-[95vh] overflow-hidden"
            ...
            >
              ...
              <div className="overflow-auto max-h-[80vh]">
                <DependencyGraph
                  todos={todos}
                  criticalPathMap={todosWithCriticalPath}
                />
              </div>
            </div>
          </div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        if (e.target.checked) {
                          ...
                          setSelectedDependencies(
                            selectedDependencies.filter((id) => id !== todo.id)
                          );
                        }
                      }}
                      className="rounded"
                  ...
                  <div className="flex-grow">
                    ...
                    <div className="text-sm text-gray-600 space-y-1">
                      ...
                      {criticalInfo?.earliestStartDate && (
                        <div>
                          Earliest Start:{" "}
                          {formatDate(criticalInfo.earliestStartDate)}
                        </div>
                      )}

                      {criticalInfo && criticalInfo.slack > 0 && (
                        <div>Slack: {criticalInfo.slack.toFixed(0)} days</div>
                      )}

                      {todo.dependencies.length > 0 && (
                        <div>
                          Dependencies:{" "}
                          {todo.dependencies.map((d) => d.title).join(", ")}
                        </div>
                      )}
...
function DependencyGraph({
  ...
  todos: Todo[];
  criticalPathMap: Map<number, TodoWithCriticalPath>;
}) {
  const nodeWidth = 140;
  const nodeHeight = 60;
  ...
  const calculateLevel = (
    ...
    const maxDepLevel = Math.max(
      ...todo.dependencies.map((d) => calculateLevel(d.id, visited))
    );
  ...
  const svgWidth = Math.max(
    800,
    maxNodesInLevel * horizontalSpacing + padding * 2
  );
  ...
  criticalPathMap.forEach((todo) => {
    if (
      todo.isCritical &&
      todo.criticalPath.length > longestCriticalPath.length
    ) {
